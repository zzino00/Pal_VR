...
public class Craft_Machine : MonoBehaviour
{
    ...
    public void SelectMethod(Weapon_Stat stat)
    {
        ...
        for (int i = 0; i < stat.craftMethod.Length; i++)// 조합식 리스트를 돌면서 
        {
            if (stat.craftMethod[i] != 0)// 조합재료가 존재할때만
            {
                ...
                Craft_InputObj input = Instantiate(inputObject, gameObject.transform).GetComponent<Craft_InputObj>();// 투입구 생성
    ...
    public void CreateWeapon()
    {
        ...
        Instantiate(newWeaponObj, gameObject.transform.position + new Vector3(0, 1, 0), gameObject.transform.rotation);// 무기 생성
    }
}
